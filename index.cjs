...
async function for_of(values, obj) {
  ...
  try {
    for (const value of values) {
      const newObj = JSON.parse(await interpreter.interpret(obj, value));
      res.push(newObj);
    }
    return res;
  } catch (ex) {
 ...
 */
function lex(input) {
  const isOperator = c => /[\(\)]/.test(c) || (c === ',' && !(/['"]/.test(input[i - 1]) && /['"]/.test(input[i + 1])));
  ...
  const parseIdentifier = (identifier = token()) => {
    ...
    if (token().type === 'operator' && token().value === '(') {
      ...
      if (token().type === 'operator' && token().value === ')') {
        ...
        } while (token().type === 'operator' && token().value === ','); // Keep parsing arguments until no more commas
